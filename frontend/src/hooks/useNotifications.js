...
export const useNotifications = (userId) => {
  ...
  const connectWebSocket = useCallback(() => {
    if (!userId) {
      console.log('🔔 useNotifications: userId não fornecido, pulando conexão WebSocket');
      return;
    }
    
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      console.log('🔔 useNotifications: WebSocket já conectado');
      return;
    }

    try {
      const wsUrl = `ws://localhost:8000/ws/notifications/${userId}`;
      console.log(`🔔 useNotifications: Conectando WebSocket para usuário ${userId}: ${wsUrl}`);
